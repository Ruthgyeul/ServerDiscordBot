...
- **📊 Host monitoring** — CPU, memory, swap, disk usage and uptime at a glance
  (`/status`), plus the heaviest processes (`/top`).
...
- **🌐 Website health checks** — HTTP status, latency and **TLS certificate
  expiry** for the sites you host (`/sites`).
...
- **Operable** — structured JSON logs to stdout, graceful shutdown on SIGTERM,
  so any process supervisor can manage it.
...
└── lib/                     # shell, permissions, embeds, autocomplete, formatting
...
| --------------------- | -------- | ----------------------------------------------------- |
| `DISCORD_TOKEN`       | ✅       | Bot token *(restart to change)*                       |
...
| `LOG_LEVEL`           |          | `trace`..`fatal` (default `info`) *(restart)*         |
| `NODE_ENV`            |          | `production` for JSON logs *(restart)*                |
...
```jsonc
{
  "monitor": {
    ...
    "checks": {                     // turn individual passes off
      "resources": true, "services": true,
      "websites": true, "certificates": true
    },
    "thresholds": {
      "cpuPercent": 85, "memoryPercent": 90, "diskPercent": 90,
  ...
  "services": [
    { "name": "portfolio", "label": "Portfolio", "unit": "portfolio.service",
      "description": "ruthgyeul.xyz", "critical": true }
  ],

  "websites": [
    { "name": "portfolio", "label": "Portfolio", "url": "https://ruthgyeul.xyz",
      "expectStatus": 200, "timeoutMs": 8000, "checkCert": true,
      "service": "portfolio" }     // links the site to the unit behind it
  ],

  "commands": [
    { "name": "disk", "label": "Disk usage", "command": "df", "args": ["-h"],
      "sudo": false, "allowArgs": false, "timeoutMs": 15000 }
  ],

  "files": [
    { "name": "nginx-error", "label": "Nginx error log",
      "path": "/var/log/nginx/error.log", "maxLines": 200 }
  ]
}
```
...
Discord users select a *key* — they never supply a unit name, argv or path.
...
and *new slash commands* or changed command definitions (`npm run deploy`).
